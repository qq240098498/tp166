...
- 账单：按账期与客户出账，查看账单总额与逐条明细，可以把账单作废；出账后运单再有改动会自动核对，差额以单独一笔调整体现
...
6. 账期按运单创建时刻所在的月份归集；运单进账单之后会被锁定，不能删除（但仍可编辑改正数据）

## 出账后改动核对

已出账账单的金额是出账当时的结论，不会因为之后改运单而变化。出账时每条明细会冻结一份快照（运单重量/体积/件数/保价/服务、对应分区单价、计费参数、客户折扣）。之后每次查看账单时，系统拿快照与当前数据按同一套**单运单计费口径**各算一次：

- 有差额的运单逐条列出：运单号、计费重量（当时 → 现在）、按当时金额、按现在金额、差额、变化原因（实际重量、体积、收件城市/分区、件数、保价、附加服务、计费参数、客户折扣）
- 差额方向：现在金额更高为**少收**（应补收），更低为**多收**（应退）
- 全部差额汇总成账单上**单独一笔调整**，只展示「原账单金额 / 调整笔 / 按现在数据应收合计」，不回写原账单金额
- 编辑已进账单的运单时，保存结果里直接带差额预警；运单卡片、详情与账单清单卡片上也会出现差额标记
- 已作废账单不参与核对；把改前的值改回去，差额自动消失
